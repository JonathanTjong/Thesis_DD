...
struct COLOR {
   ...
   COLOR(const COLOR& c) : s(c.s),last(c.last),vtx(c.vtx), beams(c.beams), extras(c.extras), counter(c.counter) {}
   COLOR(COLOR&& c) : s(std::move(c.s)),last(c.last),vtx(c.vtx), beams(c.beams), extras(c.extras), counter(c.counter) {}
   COLOR(Legal&& s,int l,int v, int beams, int extras, int counter) : s(std::move(s)),last(l),vtx(v), beams(beams), extras(extras), counter(counter) {}
...
int main(int argc,char* argv[])
{
   ...
   // const auto labels = setFrom(std::views::iota(1,K));     // using a plain set for the labels
   const auto init = [K,labels]() {   // The root state
      Legal A(K, 0);      // Noboby is colored yet (0 to everyone)
      return COLOR { std::move(A),0,0,0,0,0};
   };
   const auto target = [K]() {    // The sink state
      return COLOR { Legal{},0,K,0,0,0};
   };
   const auto lgf = [K,&labels,&bnds,&adj, beam_n](const COLOR& s,DDContext) -> GNSet {
      ...
      for(auto a : adj[s.vtx])  // [WvH] We could limit adj[i] to indices < i because of the fixed variale ordering, right? [LDM]. Yes, of course!
         if (s.s[a] != 0)           // if neighbor already has a certain color, remove from valid list
            valid.remove(s.s[a]);
      auto ub = s.last+1;  // no primal here in only relaxed
   ...
   auto stf = [K, beam_n](const COLOR& s,const int label) -> std::optional<COLOR> {
      // this prints number of transitions
      std::cout<<"1 ";

      if (s.vtx+1 == K) {
         // std::cout << "counter: " << s.counter << std::endl;      // try to catch counter at the end, need to do +1
         return COLOR { Legal {},0, K+1, 0 , s.extras, s.counter};
      }
      if (label == K+1) { //depot
         return COLOR { Legal {},0, K, 0 , 0, s.counter};
      }
      Legal B = s.s;
      ...
      if (label == K) {
         ...
         return COLOR { std::move(B), s.last, s.vtx, s.beams+1, s.extras+1, s.counter+1};
      }

      B[s.vtx] = label;
      return COLOR { std::move(B), std::max(label,s.last), s.vtx + 1, 0, s.extras, s.counter+1};
   };
   ...
   const auto smf = [](const COLOR& s1,const COLOR& s2) -> std::optional<COLOR> {
      if (s1.last == s2.last && s1.vtx == s2.vtx) {
         ...
         return COLOR { std::move(B) , s1.last, s1.vtx, std::max(s1.beams, s2.beams), std::max(s1.extras, s2.extras), std::max(s1.counter, s2.counter)};
      }
   ...
   AbstractDD::Ptr theDD = DD<COLOR,Minimize<double>,
                   decltype(target),
                   decltype(lgf),
                   decltype(stf),
                   decltype(scf),
                   decltype(smf),
                   decltype(eqs)
                   >::makeDD(init,target,lgf,stf,scf,smf,eqs,labels);
   bnds2.attach(theDD);

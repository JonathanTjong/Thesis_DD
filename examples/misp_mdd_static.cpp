...
// helper function for the max connectivity variable ordering
...
int get_max_connectivity(const std::vector<int> connectivity, const std::vector<int> degrees) {
   int max_idx = 0;
   int max_connectiviy = INT_MIN;
   int max_deg = INT_MIN;

   for (int i = 0; i < connectivity.size(); i++) {
      if (connectivity[i] > max_connectiviy || (connectivity[i] == max_connectiviy && degrees[i] > max_deg)) {
         max_connectiviy = connectivity[i];
         max_deg = degrees[i];
         max_idx = i;
      }
   }
...
int main(int argc,char* argv[])
{
   ...
   // << this variable ordering was not used in the end, as it did not perform as well >>
   // max degree heuristic static ordering
   ...
   // simple max degree static ordering
   ...
   // actual neighbors list, will be modified after var ordering
   ...
   const auto lgf = [beam_n, top](const MISPMDD& s, DDContext)  {
      if (s.sel.size() > beam_n) {
         ...
         return out;       //!! watch out, extra branch maybe appear in solution
      }
      else {
         GNSet u = s.sel;
         return u;
   ...
   auto myStf = [top,&neighbors, beam_n, top](const MISPMDD& s,const int label) -> std::optional<MISPMDD> {
      if (s.transitions == -1) {
         ...
         if (label == top) {
            ...
            return MISPMDD { std::move(out), empty ? -1 : s.transitions+1};   // keep previous s.transitions
         }
         else {
   ...
   BAndB engine(DD<MISPMDD,Maximize<double>, // to maximize
                ...
                decltype(eqs),
                decltype(local)
                // >::makeDD(myInit,myTarget,lgf,myStf,scf,smf,eqs,labels,local,sDom),w);

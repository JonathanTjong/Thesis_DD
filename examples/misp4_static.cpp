...
int main(int argc,char* argv[])
{
   ...
   int ordering_selection = argc>=4 ? atoi(argv[3]) : 0;
   ...
   Bounds bnds([&es, &reverse_sorted](const std::vector<int>& inc)  {
      bool ok = true;
      for(const auto& e : es) {
         bool v1In = (reverse_sorted[e.a] < (int)inc.size()) ? inc[reverse_sorted[e.a]] : false;
         bool v2In = (reverse_sorted[e.b] < (int)inc.size()) ? inc[reverse_sorted[e.b]] : false;
         if (v1In && v2In) {
   ...
   const auto local = [&weight, &neighbors](const MISP& s, LocalContext) -> double {
      return sum(s.sel,[&weight](auto v) { return weight[v];});
   };
   const auto sDom = [](const MISP& a,const MISP& b) -> bool {
      GNSet bset = b.sel;
      GNSet aset = a.sel;
      GNSet inter = bset.interWith(a.sel);
      return inter.empty();
   };
